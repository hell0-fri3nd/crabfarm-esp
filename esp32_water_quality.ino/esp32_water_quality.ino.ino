...
void setTDS_15PPT_Point() {
  ...
  float sum = 0;
  for (int i = 0; i < 10; i++) { sum += analogRead(TDS_PIN); delay(50); }
  tds_15ppt_raw = sum / 10.0;
  Serial.printf("TDS 15ppt point saved! ADC = %.2f\n", tds_15ppt_raw);
  Serial.println("Step 2: Place sensor in 25 ppt (25000 ppm) solution");
...
float readDO() {
  ...
  return (adc * ADC_REF / ADC_MAX * DIV_RATIO / 5.0) * 20.0;
}
...
float readPH() {
  ...
  return sensors.temp_ok ? adcToPH(adc, sensors.temperature) : (PH_NEUTRAL - (adc * ADC_REF / ADC_MAX * DIV_RATIO)) / PH_SLOPE_VAL;
}

float readTDS() {
  if (!sensors.tds_ok) return 0.0;
  int adc = analogRead(TDS_PIN);
  return sensors.temp_ok ? adcToTDS(adc, sensors.temperature) : (adc * ADC_REF / ADC_MAX * DIV_RATIO) * 1000;
}
...
float readTurbidity() {
  ...
  return constrain(5000 - ((adc * ADC_REF / ADC_MAX * DIV_RATIO) * 1125), 0.0, 4000.0);
}
...
void cancelTest() {
    if (state != IDLE) {
        ...
        setRelay(4, false);  // Water pump
        setRelay(5, false);  // Solenoid
        
        // Reset state
        state = IDLE;
        
        // Send cancellation message to web clients
  ...
  switch(state) {
    case SENSOR_RELAY_ON:
      if (millis() - state_start > 500) {
        printTestStep(1, "Turn ON Relay 1,2,3 (Sensors Power)");
    ...
    case WAIT_WATER:
      ...
      if (sensors.water_detected) {
        ...
        state = STABILIZE;  // Continue to stabilization even without water
        state_start = millis();
        Serial.println("\n⏳ STEP 5: Waiting 10 minutes for sensor stabilization...");
    ...
    case SENSOR_RELAY_OFF:
      if (millis() - state_start > 500) {
        printTestStep(8, "Turn OFF Relay 1,2,3 (Sensors Power)");
        allSensorRelays(false);
        state = SOLENOID_ON;
        state_start = millis();
      }
...
void sendHTML() {
  ...
  html += "<div class='relay-row'><span class='relay-name'>Relay 1</span><span class='relay-desc'>Sensor 1 Power</span>";
  ...
  html += "<div class='relay-row'><span class='relay-name'>Relay 2</span><span class='relay-desc'>Sensor 2 Power</span>";
  ...
  html += "<div class='relay-row'><span class='relay-name'>Relay 3</span><span class='relay-desc'>Sensor 3 Power</span>";
  ...
  html += "<div class='card'><h3>💧 TDS Sensor</h3><div class='reading'>TDS: <span id='tdsCurrent'>--</span> ppm</div><div id='tdsCalStep'></div></div>";
  ...
  html += "if(ts===1)document.getElementById('tdsCalStep').innerHTML='<div class=\"cal-step\"><h4>Step 1/2: Set 15ppt (15000 ppm)</h4><button class=\"btn-success\" onclick=\"sendCommand(\\'/cal/tds/set15ppt\\')\">Set 15ppt</button><button onclick=\"sendCommand(\\'/cal/tds/cancel\\')\">Cancel</button></div>';";
  html += "else if(ts===2)document.getElementById('tdsCalStep').innerHTML='<div class=\"cal-step\"><h4>Step 2/2: Set 25ppt (25000 ppm)</h4><button class=\"btn-success\" onclick=\"sendCommand(\\'/cal/tds/set25ppt\\')\">Set 25ppt</button><button onclick=\"sendCommand(\\'/cal/tds/cancel\\')\">Cancel</button></div>';";
